...
- 本地验证基线：`:app:testDebugUnitTest` 与 `:app:assembleDebug`。
...
- 多模态图片基线：图片导入 app-private storage、消息级附件持久化、附件映射进 LLM message，以及 OpenAI-compatible provider payload 支持。
- Memory 系统：实时 consolidation、分层召回、冲突感知 fact 治理、排序检索，以及内置 fact / summary 管理。
...
- Memory 已具备实时刷新和治理能力，但尚未引入更重的 provenance / confidence 模型。
...
- Memory 可解释性：更明确的来源说明、置信度展示、纠正记录与回溯信息。
...
- Memory 已升级为：回合后实时刷新、分层召回、冲突感知 fact 替换、排序式 lookup，以及 Memory 页面上的 fact / summary 管理操作。
- Memory 页面和相关 UI 已完成一轮可用性优化，更适合作为内置治理入口使用。

## 建议的下一阶段

1. 继续做文档与调试文案整理，让项目状态无需依赖聊天历史也能快速理解。
2. 补充更清晰的 memory 来源说明 / 置信度展示，以及可选的更丰富纠正流程。
3. 进一步硬化 MCP，包括 auth / timeout / health-check / backoff。
4. 明确是停留在 Android 原生核心阶段，还是继续推进到 channels / bridge / CLI 与更广 Python 生态对齐。
